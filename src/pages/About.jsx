...
export const Title = ({ secondary, children }) => (
  <h2 className={`${styles.title} ${secondary ? styles.secondaryTitle : ''}`}>{children}</h2>
);
const Text = ({ children }) => <p className={styles.text}>{children}</p>;
const Link = ({ children, ...props }) => <a className={styles.link} {...props}>{children}</a>;
const List = ({ children }) => <li className={styles.list}>{children}</li>;
